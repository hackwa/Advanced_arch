...
--               
   ...
   100 us as determined by Processor Expert Component Configuration.
...
//    A. Display timer and flag
      ...
      if (display_timer == 1)
    	 {
    	 // GPIO_HAL_TogglePinOutput(PTB,18);
...
//   Generate Outputs  ************************************
    ...
    // Create an 0.5 second RED LED heartbeat here. 
      led_timer--;
      if (led_timer == 80)
    	  GPIO_HAL_ClearPinOutput(PTB,18);
      if (led_timer == 0)
    	  GPIO_HAL_SetPinOutput(PTB,18);
     // GPIO_HAL_TogglePinOutput(PTB,18);
