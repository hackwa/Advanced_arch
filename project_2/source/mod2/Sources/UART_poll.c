...
#define OERR (UART0_S1 & UART0_S1_OR_MASK)   // Overrun Error bit
#define CREN (UART0_C2 & UART0_C2_RE_MASK)   // continuous receive enable bit
#define RCREG UART0_D                        // Receive Data Register
#define FERR (UART0_S1 & UART0_S1_FE_MASK)   // Framing Error bit
#define RCIF (UART0_S1 & UART0_S1_RDRF_MASK)    // Receive Interrupt Flag (full)
#define TXIF (UART0_S1 & UART0_S1_TDRE_MASK)    // Transmit Interrupt Flag (empty)
#define TXREG UART0_D                        // Transmit Data Register
#define TRMT (UART0_S1 & UART0_S1_TC_MASK)    // Transmit Shift Register Empty
                        ...
                        // received data or data to transmit
{
   ...
   if ( OERR )             // if an overrun error, clear it and continue.
   {
      ...
      UART0_C2 = UART0_C2 & (!UART0_C2_RE_MASK);
      UART0_C2 = UART0_C2 | UART0_C2_RE_MASK;
   }
...
*******************************************************************************/
void UART_direct_msg_put(UCHAR *str)
{
   while( *str != '\0' )
   {
      TXREG = *str++;
