      ...
      case(REGISTER):
         {
          if (display_flag == 1)
          {
        	  ...
        	  union register_type reg,internal ;
             ...
             UART_msg_put("Result: ");
             UART_hex_put(reg.byte[3]);
             UART_hex_put(reg.byte[2]);
             UART_hex_put(reg.byte[1]);
             UART_hex_put(reg.byte[0]);
//         UART_msg_put("  0x");
//             UART_hex_put(internal.byte[3]);
//             UART_hex_put(internal.byte[2]);
//             UART_hex_put(internal.byte[1]);
//             UART_hex_put(internal.byte[0]);
             UART_msg_put("\r\n");
             display_flag = 0;
           }
         }
